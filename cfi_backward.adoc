...
[start=3]
.  If `pte.v = 0` or if any bits of encodings that are reserved for future
...
.  Otherwise, the PTE is valid. If `pte.r = 1` or `pte.w = 1` or `pte.x = 1`,
...
.  A leaf PTE has been found. If the memory access is by a shadow stack
